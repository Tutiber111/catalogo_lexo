...
begin
  ...
  if actor_salesman_code is null then
    raise exception 'Only salesmen with an assigned code can view these requests.'
      using errcode = '42501';
  end if;
...
begin
  ...
  if actor_salesman_code is null then
    raise exception 'Only salesmen with an assigned code can approve requests.'
      using errcode = '42501';
  end if;
  ...
  if not found then
    raise exception 'Request not found or not assigned to this salesman.'
      using errcode = 'P0002';
  end if;
end;
$$;
